...
public class ClientDemo5 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        context.setHandler(ClientDemo5::handleRequest);
        server.start();
...
//        cl.testIt(TRUSTSTORE);
    ...
    private static void handleRequest(HttpExchange exchange) throws IOException {
        ...
        Map<String, List<String>> cors = new HashMap<String, List<String>>();
        cors.put("Access-control-allow-origin", Arrays.asList(new String[]{"*"}));
        cors.put("Access-control-allow-methods", Arrays.asList(new String[]{"GET, POST, OPTIONS"}));
        if (exchange.getRequestMethod().equalsIgnoreCase("OPTIONS")) {
            Map<String, List<String>> requestHeader = exchange.getRequestHeaders();
            requestHeader.put("Access-control-allow-origin", Arrays.asList(new String[]{"*"}));
            requestHeader.put("Access-control-allow-methods", Arrays.asList(new String[]{"GET, POST, OPTIONS"}));
            setResponseHeader(exchange, requestHeader);
        ...
        try {
            ...
            // Add CORS Headers
            System.out.println(exchange.getRequestHeaders().get("Host"));
            if (exchange.getRequestHeaders().get("Host") != null
                    && exchange.getRequestHeaders().get("Host").size() != 0) {
                // [192.168.1.133:8500]
                String reqHost = exchange.getRequestHeaders().get("Host").get(0);
                if (reqHost != null && reqHost.indexOf("192.168") != -1) {
                    //setResponseHeader(exchange, cors);
                    addCorsHeader(content.getHeaders(), cors);
                }
            }
...
//          || key.equalsIgnoreCase("Host")
    ...
    private static void addCorsHeader(Map<String, List<String>> headers, Map<String, List<String>> cors) {
        if (cors == null || cors.keySet().size() == 0) return;
        for (Map.Entry<String, List<String>> entry : cors.entrySet()) {
            String key = entry.getKey();
            List<String> value = entry.getValue();
            if (key == null || key.equalsIgnoreCase("null")
//          || key.equalsIgnoreCase("Referer")
